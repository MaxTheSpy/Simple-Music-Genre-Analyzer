            ...
            for tag in ['genre', 'GENRE', 'TCON']:  # Common tags for genre
                if tag in audio.tags:
                    ...
                    if isinstance(genre, list):
                        genre = genre[0]
                    # Convert to string, split by ';', and normalize
...
def map_to_simplified_genres(complex_genres):
    """Map complex genres to simplified categories using keyword matching."""
    simplified_keywords = {
        #
        # A
        "Ambient": ["ambient"],
...
def calculate_genre_distribution(audio_files, simplified=False):
    ...
    for file in audio_files:
        ...
        genres.extend(genre_list)  # Add all genres from the list
...
def run_analysis(directory, simplified, output_widget):
    ...
    if genre_distribution:
        ...
        # Add separator for tagged and untagged genres
...
def create_ui():
    root = tk.Tk()
    root.title("Simple Music Genre Analyzer")
    ...
    def start_analysis():
        ...
        dir_path = directory_var.get()
        simplified = simplified_var.get()
        run_analysis(dir_path, simplified, output_text)
        root.after(5000, lambda: analyze_button.config(text="Analyze", state=tk.NORMAL))

    directory_var = tk.StringVar()
    simplified_var = tk.BooleanVar(value=False)

    tk.Label(main_frame, text="Music Directory:").grid(row=0, column=0, padx=5, pady=5, sticky="e")
    tk.Entry(main_frame, textvariable=directory_var, width=50).grid(row=0, column=1, padx=5, pady=5, sticky="ew")
    tk.Button(main_frame, text="Browse", command=browse_directory).grid(row=0, column=2, padx=5, pady=5)

    tk.Checkbutton(main_frame, text="Show Simplified Genres", variable=simplified_var).grid(row=1, column=0, columnspan=3, padx=5, pady=5)

    analyze_button = tk.Button(main_frame, text="Analyze", command=start_analysis)
    analyze_button.grid(row=2, column=0, columnspan=3, pady=10)

    output_text = scrolledtext.ScrolledText(main_frame, wrap=tk.WORD)
